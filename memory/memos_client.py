...
class MemosClient:
    ...
    def _post(self, endpoint: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            res = requests.post(url=url, headers=self.headers, data=json.dumps(payload))
            res.raise_for_status() # 检查 HTTP 错误
            return res.json()
        except requests.exceptions.RequestException as e:
            return {"code": -1, "message": f"Request failed: {str(e)}"}

    def add_messages(self, user_id: str, conversation_id: str, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        """
        ...
        return self._post("add/message", data)

    def get_history(self, user_id: str, conversation_id: str, limit: int = 10) -> List[Dict[str, str]]:
        """
        ...
        result = self._post("get/message", data)
    ...
    def search_memory(
            ...
            result = self._post("search/memory", data)
...
if __name__ == "__main__":
    ...
    res_add = client.add_messages(user_id, conv_id, new_msgs)
    print(f"Add Result: {res_add}")

    print("\n--- 2. 获取历史 (清洗版) ---")
    history = client.get_history(user_id, conv_id, limit=5)
    print(json.dumps(history, ensure_ascii=False, indent=2))

    print("\n--- 3. 搜索记忆 ---")
    # 假设用户之前说过不喜欢吃太辣的
    query = "还是给我推荐点不辣的吧，我肠胃不好。"
    memory_context = client.search_memory(user_id, query, conv_id)
